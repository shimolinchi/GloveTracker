...
/// @brief This gets called when the client is connected to manus core. It sends the raw device data coming from core. 
/// @param p_RawDeviceDataStreamInfo contains the meta data on how much data regarding the raw devices we need to get from the SDK.
...
ClientReturnCode SDKClient::RegisterAllCallbacks()
{
	...
	// Register the callback for when manus core is sending raw device data
	// it is optional, but without it you can not see any resulting raw device data.
	// see OnRawDeviceDataStreamCallback for more details.
	const SDKReturnCode t_RegisterRawDeviceDataCallbackResult = CoreSdk_RegisterCallbackForRawDeviceDataStream(*OnRawDeviceDataStreamCallback);
	if (t_RegisterRawDeviceDataCallbackResult != SDKReturnCode::SDKReturnCode_Success)
	{
		ClientLog::error("Failed to register callback function for processing raw device data from Manus Core. The value returned was {}.", (int32_t)t_RegisterRawDeviceDataCallbackResult);
		return ClientReturnCode::ClientReturnCode_FailedToInitialize;
	}
...
ClientReturnCode SDKClient::UpdateBeforeDisplayingData()
{
	...
	m_RawDeviceDataMutex.lock();
	if (m_NextRawDeviceData != nullptr)
	{
		if (m_RawDeviceData != nullptr)delete m_RawDeviceData;
		m_RawDeviceData = m_NextRawDeviceData;
		m_NextRawDeviceData = nullptr;
	}
	m_RawDeviceDataMutex.unlock();
...
ClientReturnCode SDKClient::DisplayingData()
{
	...
	GO_TO_DISPLAY('U', DisplayingUsers);
	GO_TO_DISPLAY('E', DisplayingRawDeviceData);
...
ClientReturnCode SDKClient::DisplayingUsers()
{
	ClientLog::print("[Q] Back  <<Users>> [ESC] quit");
	ClientLog::print("<<Users>> [Z] Disable Auto-Assignment");
	ClientLog::print("<<Users>> [X] Enable Auto-Assignment");
	ClientLog::print("<<Users>> [A] Add User");
	ClientLog::print("<<Users>> [R] Remove User");
	ClientLog::print("<<Users>> [D] Assign Dongle");
	ClientLog::print("<<Users>> [U] Unassign Dongle");
	ClientLog::print("<<Users>> [G] Assign Glove");
	ClientLog::print("<<Users>> [W] Unassign Glove");
	ClientLog::print("<<Users>> [I] Move User up");
...
ClientReturnCode SDKClient::DisplayingRawDeviceData()
{
	ClientLog::print("[Q] Back  <<Raw device data>> [ESC] quit");
...
/// Since our console cannot render this data visually in 3d (its not in the scope of this client)
/// we only display a little bit of data of the device we receive.
/// The intended order of the sensors is as follows: thumb, index, middle, ring, pinky.
...
void SDKClient::HandleGloveCalibrationCommands()
{
	...
	bool t_GloveConnected = false;
	for (size_t i = 0; i < m_Landscape->gloveDevices.gloveCount; i++)
	{
		if (m_CalibrationGloveId != m_Landscape->gloveDevices.gloves[i].id)
			continue;

		t_GloveConnected = true;
		break;
	}
	...
	if (GetKeyDown('K')) // Save Calibration
	{
		uint32_t t_Size;
		CoreSdk_GetGloveCalibrationSize(m_CalibrationGloveId, &t_Size);
		unsigned char* t_CalibrationBytes = new unsigned char[t_Size];
		CoreSdk_GetGloveCalibration(t_CalibrationBytes, t_Size);
	...
	if (GetKeyDown('L')) // Load Calibration
	{
		std::string t_DirectoryPathString = GetDocumentsDirectoryPath_UTF8();
		...
		CoreSdk_SetGloveCalibration(m_CalibrationGloveId, t_CalibrationData, t_FileLength, &t_Result);
	}
}

void SDKClient::HandleUserCommands()
{
	if (GetKeyDown('Z'))
	{
		if (CoreSdk_SetAutoUserAssignment(false) == SDKReturnCode_Success)
		{
			ClientLog::print("Disabled Auto Assignment");
		}
		else
		{
	...
	if (GetKeyDown('X'))
	{
		if (CoreSdk_SetAutoUserAssignment(true) == SDKReturnCode_Success)
		{
			ClientLog::print("Enabled Auto Assignment");
		}
		else
		{
...
void SDKClient::RemoveUser()
{
	...
	//Gets the ID of the last user in the list
	UserLandscapeData t_UserToRemove = m_Landscape->users.users[m_Landscape->users.userCount - 1];
...
void SDKClient::AssignDongle()
{
	...
	uint32_t t_UserId;
	bool t_UserFound = false;
	//Look for a user with no dongle assigned
	for (size_t i = 0; i < m_Landscape->users.userCount; i++)
	{
		if (m_Landscape->users.users[i].dongleID == 0) {
			t_UserId = m_Landscape->users.users[i].id;
			t_UserFound = true;
			break;
		}
	}
	...
	ClientLog::print("Something went wrong assigning dongle {} to user {}.", t_DongleId, t_UserId);
}

void SDKClient::UnassignDongle()
{
	if (m_Landscape->users.userCount < 1)
	{
...
void SDKClient::AssignGloveToUser()
{
	...
	for (size_t i = 0; i < m_Landscape->users.userCount; i++)
	{
		...
		{
		case Side_Left:
			if (m_Landscape->users.users[i].leftGloveID == 0)
			{
				t_UserId = m_Landscape->users.users[i].id;
			}
			break;
		case Side_Right:
			if (m_Landscape->users.users[i].rightGloveID == 0)
			{
				t_UserId = m_Landscape->users.users[i].id;
			}
			break;
		...
		// Break out of the loop because we found a user.
		if (t_UserId != 0)
		{
			break;
		}
...
void SDKClient::UnassignGlove()
{
	...
	for (size_t i = 0; i < m_Landscape->users.userCount; i++)
	{
		...
		else if (m_Landscape->users.users[i].rightGloveID != 0)
		{
			t_GloveFound = true;
			t_UserId = m_Landscape->users.users[i].id;
			t_Side = Side_Right;
	...
	if (!t_GloveFound)
	{
		ClientLog::warn("No glove to unassign was found.");
	}

	std::string t_SideString = "";
	switch (t_Side)
	{
	case Side_Left:
		t_SideString = "left";
	...
	ClientLog::warn("Something went wrong unassigning {} glove from user {}.", t_SideString, t_UserId);
}

void SDKClient::MoveUserUp()
{
	if (m_Landscape->users.userCount < 1)
	{
	...
	// For this sample we move the most recently created user around. This is always the user with the highest ID;
	uint32_t t_HighestID = 0;

	for (size_t i = 0; i < m_Landscape->users.userCount; i++)
	{
		if (m_Landscape->users.users[i].id > t_HighestID)
		{
			t_HighestID = m_Landscape->users.users[i].id;
		}
	}
...
void SDKClient::RenameUser()
{
	...
	// For this sample we just add "but different" to the end of the username already in use
	std::string t_NewName = m_Landscape->users.users->name;
	t_NewName.append(" but different");

	// -1 because the Args use a c style string which also needs a spot for a null terminator
	if (t_NewName.length() > MAX_NUM_CHARS_IN_USERNAME - 1)
	{
		t_NewName.resize(MAX_NUM_CHARS_IN_USERNAME - 1);
	}

	uint32_t t_UserToRename = m_Landscape->users.users[0].id;
...
{
	for (int i = 0; i < m_Landscape->users.userCount; i++)
	{
		ClientLog::print("User: {}, ID: {}, Dongle ID: {}, Left Glove ID: {}, Right Glove ID: {}",
			m_Landscape->users.users[i].name,
			m_Landscape->users.users[i].id,
			m_Landscape->users.users[i].dongleID,
...
bool SDKClient::GetFirstAvailableDongleId(uint32_t* p_DongleId)
{
	std::vector<uint32_t> t_AssignedDongles;

	for (size_t i = 0; i < m_Landscape->users.userCount; i++)
	{
		if (m_Landscape->users.users[i].dongleID != 0) {
			t_AssignedDongles.push_back(m_Landscape->users.users[i].dongleID);
		}
	}

	for (size_t i = 0; i < m_Landscape->gloveDevices.dongleCount; i++)
	{
		bool t_DongleAssigned = false;
		uint32_t t_DongleId = m_Landscape->gloveDevices.dongles[i].id;

		for (size_t j = 0; j < t_AssignedDongles.size(); j++)
		{
			if (t_DongleId == t_AssignedDongles[j])
			{
				t_DongleAssigned = true;
				break;
			}
		}
...
bool SDKClient::GetFirstAvailableGlove(GloveLandscapeData* p_Glove)
{
	std::vector<uint32_t> t_AssignedGloves;

	for (size_t i = 0; i < m_Landscape->users.userCount; i++)
	{
		if (m_Landscape->users.users[i].leftGloveID != 0) {
			t_AssignedGloves.push_back(m_Landscape->users.users[i].leftGloveID);
		}

		if (m_Landscape->users.users[i].rightGloveID != 0) {
			t_AssignedGloves.push_back(m_Landscape->users.users[i].rightGloveID);
		}
	}

	for (size_t i = 0; i < m_Landscape->gloveDevices.gloveCount; i++)
	{
		bool t_GloveAssigned = false;
		uint32_t t_GloveId = m_Landscape->gloveDevices.gloves[i].id;

		for (size_t j = 0; j < t_AssignedGloves.size(); j++)
		{
			if (t_GloveId == t_AssignedGloves[j])
			{
				t_GloveAssigned = true;
				break;
			}
		}
		...
		if (!t_GloveAssigned)
		{
			*p_Glove = m_Landscape->gloveDevices.gloves[i];
			return true;
		}
	}
...
ErgonomicsData SDKClient::GetGloveErgoData(bool is_left)
{
	if (is_left)
	{
		return m_LeftGloveErgoData;
	}
	else
	{
