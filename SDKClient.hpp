...
/// @brief Used to store all the raw device data coming from Core.
class ClientRawDeviceDataCollection
{
public:
	std::vector<RawDeviceData> rawDeviceData;
};
...
public:
	...
	static void OnRawDeviceDataStreamCallback(const RawDeviceDataInfo* const p_RawDeviceDataStreamInfo);
...
protected:
	...
	virtual ClientReturnCode DisplayingUsers();
	virtual ClientReturnCode DisplayingRawDeviceData();
	...
	void AssignDongle();
	void UnassignDongle();
	void AssignGloveToUser();
	void UnassignGlove();
	void MoveUserUp();
	void MoveUserDown();
	void RenameUser();
	void PrintUsersLandscapeData();

	bool GetFirstAvailableDongleId(uint32_t* p_DongleId);

	bool GetFirstAvailableGlove(GloveLandscapeData* p_Glove);
...
protected:
	...
	std::mutex m_RawDeviceDataMutex;
	ClientRawDeviceDataCollection* m_RawDeviceData = nullptr;
	ClientRawDeviceDataCollection* m_NextRawDeviceData = nullptr;
